					...
					uploadCallback : function($uploadBox, resultObj) {
						if (resultObj) {
							...
							if (resultObj.success) {
								...
								$(".xupload-list-banner-image img:first-child").remove();
							...
							clubEditFormValidator.element($hiddenField);
						} else {
							alert("上传失败!");
						}
					},
...
var clubEditFormValidator = $(document.clubEditForm).validate(validateOptions);

/**
 * 修改知识类别
 */
function editClub(el){
	var valid = clubEditFormValidator.form();
	if(valid){
		showLoadingText(el);
		setTimeout(function(){
			document.clubEditForm.submit();
		},1000);
	}
}
 ...
 */
//$(function(){
//	var selectTag=$("#areainfo")[0];
//	var selectvalue=$("#selectvalue").val();
//	for(var i=0;i<selectTag.length;i++){
//		if(selectTag.options[i].value==selectvalue){
