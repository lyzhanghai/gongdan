		...
		.xupload(
				{
					...
					uploadCallback : function($uploadBox, resultObj) {
						if (resultObj) {
							...
							couponAddFormValidator.element($hiddenField);
						} else {
							alert("上传失败!");
						}
					},
...
var couponAddFormValidator = $(document.couponAddForm)
...
function saveCoupon(el) {
	var form = document.couponAddForm;
	var valid = couponAddFormValidator.form();
	if (valid) {
		if (isSubmitable(el)) {
			showLoadingText(el);
			setTimeout(function() {
