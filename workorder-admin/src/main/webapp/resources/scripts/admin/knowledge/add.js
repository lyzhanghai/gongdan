...
var knowledgeAddFormValidator = $(document.knowledgeAddForm).validate(validateOptions);

/**
 * 添加知识
 */
function saveKnowledge(el) {
	var valid = knowledgeAddFormValidator.form();
	if(valid){
		showLoadingText(el);
		setTimeout(function(){
			document.knowledgeAddForm.submit();
		},1000);
	}
}
