...
public class RedisUtil extends AbstractConstants{  
	...
	private static final String REDIS_IP= valueOf("127.0.0.1");
	
	@ConstValue("${redis.shiro.port}")
	private static final int REDIS_PORT= 6379;
	
	@ConstValue("${redis.shiro.timeout}")
    ...
    private RedisUtil() {  
        if (pool == null) {  
            ...
            config.setMaxWaitMillis(300000L);  
            config.setTestOnBorrow(true);  
            System.out.println(REDIS_IP+"---"+REDIS_PORT+"---"+TIME_OUT);
    ...
    public static void main(String... args) throws Exception {
        ...
        if (pool == null) {  
            ...
            config.setMaxWaitMillis(300000L);  
            config.setTestOnBorrow(true);  
            pool = new JedisPool(config, "127.0.0.1", 6379, 500) ;
        }  
